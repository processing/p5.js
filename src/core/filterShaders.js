      ...
      return renderer.baseFilterShader().modify(
        ({ p5 }) => {
          ...
          p5.getColor((inputs, canvasContent) => {
            ...
            const randomOffset =
              (spacing - 1.0) *
              p5.mix(-0.5, 0.5, random(uv * inputs.canvasSize));
            for (let i = 0; i < numSamples; i++) {
              const sample =
                i * spacing - (numSamples - 1.0) * 0.5 * spacing + randomOffset;
              const sampleCoord =
                uv + (p5.vec2(sample, sample) / inputs.canvasSize) * direction;
              const weight = quadWeight(
                sample,
                (numSamples - 1.0) * 0.5 * spacing
              );
              ...
              avg +=
                weight *
                texSample *
                p5.vec4(texSample.a, texSample.a, texSample.a, 1);
              total += weight;
            }
