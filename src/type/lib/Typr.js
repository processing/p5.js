...
Typr['U'] = (function () {
  ...
  var wha = [
      0x9, 0xa, 0xb, 0xc, 0xd, 0x20, 0x85, 0xa0, 0x1680, 0x180e, 0x2028, 0x2029,
      0x202f, 0x2060, 0x3000, 0xfeff
    ],
    whm = {};
  ...
  function glyphToPath(font, gid, noColor, axs) {
    ...
    var strike = null;
    if (CBLC && upng)
      for (var i = 0; i < CBLC.length; i++)
        if (CBLC[i][0] <= gid && gid <= CBLC[i][1]) strike = CBLC[i];
  ...
  function pathToSVG(path, prec) {
    ...
    var x = 0,
      ...
      ex = 0,
      ey = 0, // error between perfect and output coords
      mx = 0,
  ...
  function mipmapB(buff, w, h, hlp) {
    var nw = w >> 1,
      nh = h >> 1;
    var nbuf =
      hlp && hlp.length == nw * nh * 4 ? hlp : new Uint8Array(nw * nh * 4);
  ...
  function pathToContext(path, ctx) {
    ...
    for (var j = 0; j < cmds.length; j++) {
      ...
      if (cmd == 'M') {
        ...
        var scl =
          (Math.sqrt(Math.abs(tr['a'] * tr['d'] - tr['b'] * tr['c'])) *
        ...
        __ct.putImageData(
          new ImageData(new Uint8ClampedArray(nbmp.buffer), w, h),
          0,
          0
        );
        ctx.save();
