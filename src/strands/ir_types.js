...
export const DataType = {
  ...
  assign_on_use: {
    fnName: null,
    baseType: BaseType.ASSIGN_ON_USE,
    dimension: null,
    priority: -2
  },
  sampler2D: {
...
export const structType = function (hookType) {
  ...
  for (const prop of T.properties) {
    ...
    structType.properties.push({ name: prop.name, dataType: propType });
  }
  return structType;
};
...
export function typeEquals(nodeA, nodeB) {
  return (
    nodeA.dimension === nodeB.dimension && nodeA.baseType === nodeB.baseType
  );
}
