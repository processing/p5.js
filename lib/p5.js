...
amdclean['p5Shader'] = function (require, core, p5Graphics3D) {
  var p5 = core;
  p5.Shader = function () {
    ...
    var fragmentHeader = 'precision highp float;\n' + header;
    ...
    this.gl.attachShader(this.program, this._compileSource(this.gl.VERTEX_SHADER, vertexSource));
    this.gl.attachShader(this.program, this._compileSource(this.gl.FRAGMENT_SHADER, fragmentSource));
    ...
    var isSampler = {};
    this._regexMap(/uniform\s+sampler(1D|2D|3D|Cube)\s+(\w+)\s*;/g, vertexSource + fragmentSource, function (groups) {
  ...
  p5.Shader.prototype.setUniforms = function (uniforms) {
    ...
    for (var name in uniforms) {
      var location = this.uniformLocations[name] || this.gl.getUniformLocation(this.program, name);
      if (!location) {
        continue;
      }
      ...
      if (this._isArray(value)) {
        ...
        (this.isSampler[name] ? this.gl.uniform1i : this.gl.uniform1f).call(this.gl, location, value);
      } else {
  ...
  p5.Shader.prototype.draw = function (mesh, mode) {
    this.drawBuffers(mesh.vertexBuffers, mesh.indexBuffers[mode === this.gl.LINES ? 'lines' : 'triangles'], arguments.length < 2 ? this.gl.TRIANGLES : mode);
    return this;
  };
  p5.Shader.prototype.drawBuffers = function (vertexBuffers, indexBuffer, mode) {
    ...
    for (var attribute in vertexBuffers) {
      ...
      var location = this.attributes[attribute] || this.gl.getAttribLocation(this.program, attribute.replace(/^(gl_.*)$/, this.LIGHTGL_PREFIX + '$1'));
      ...
      this.gl.vertexAttribPointer(location, buffer.buffer.spacing, this.gl.FLOAT, false, 0, 0);
    ...
    if (length && (!indexBuffer || indexBuffer.buffer)) {
      if (indexBuffer) {
        ...
        this.gl.drawElements(mode, indexBuffer.buffer.length, this.gl.UNSIGNED_SHORT, 0);
      } else {
  ...
  p5.Shader.prototype._fix = function (header, source) {
    ...
    this._regexMap(/\bgl_\w+\b/g, header, function (result) {
      if (!(result in replaced)) {
        source = source.replace(new RegExp('\\b' + result + '\\b', 'g'), this.LIGHTGL_PREFIX + result);
  ...
  return p5.Shader;
}({}, amdclean['core'], amdclean['p5Graphics3D']);
amdclean['p5Graphics3D'] = function (require, core, shaders, p5Shader, p5Graphics) {
  var p5 = core;
  var shaders = shaders;
  ...
  p5.Graphics3D.prototype.background = function () {
    ...
    gl.clearColor(_col.color_array[0] / 255, _col.color_array[1] / 255, _col.color_array[2] / 255, _col.color_array[3] / 255);
    gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
  };
...
}({}, amdclean['core'], amdclean['shaders'], amdclean['p5Shader'], amdclean['p5Graphics']);
