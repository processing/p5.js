...
amdclean['helpers'] = function (require, core) {
  ...
  function report(message, func) {
    console.log('%c' + message + ' [http://p5js.org/reference/#p5/' + func + ']', 'background-color:#ED225D;color:#FFF;padding:2px;');
  }
  var class2type = {};
  ...
  p5.prototype._validateParameters = function (func, args, types) {
    if (isArray(types[0])) {
      ...
      if (diff > 0) {
        report('Try ' + func + '(' + types[tindex].join(', ') + '). ' + func + ' takes different numbers of parameters depending on ' + 'what you want to do, but it doesn\'t know what to do with ' + args.length + ' of them. ', func);
      }
    } else {
      if (args.length !== types.length) {
        report(func + ' is expecting ' + types.length + ' parameters, we found ' + args.length + '.', func);
      }
    }
    if (!isArray(types[0])) {
      types = [types];
    ...
    for (var format = 0; format < types.length; format++) {
      for (var p = 0; p < types[format].length && p < args.length; p++) {
        ...
        if (getType(args[p]) === 'undefined') {
          report('It looks like ' + func + ' received an empty variable in spot #' + (p + 1) + '.', func);
        } else if ([
            'Number',
            'Integer',
            'Number/Constant'
          ].indexOf(type) > -1 && !isNumeric(args[p]) || type === 'String' && getType(args[p]) !== 'string') {
          report(func + ' was expecting a ' + type + ' for parameter #' + (p + 1) + ', received "' + args[p] + '" instead.', func);
        }
      }
    }
  };
