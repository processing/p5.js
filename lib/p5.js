...
amdclean['shape2d_primitives'] = function (require, core, canvas, constants) {
  ...
  p5.prototype.arc = function (x, y, width, height, start, stop, mode) {
    ...
    var ctx = this.drawingContext;
    var vals = canvas.arcModeAdjust(x, y, width / 2, height / 2, this._ellipseMode);
    ...
    curves.forEach(function (curve, index) {
      ...
      ctx.bezierCurveTo(vals.x + curve.x2 * vals.w, vals.y + curve.y2 * vals.h, vals.x + curve.x3 * vals.w, vals.y + curve.y3 * vals.h, vals.x + curve.x4 * vals.w, vals.y + curve.y4 * vals.h);
    });
    if (this._doFill) {
