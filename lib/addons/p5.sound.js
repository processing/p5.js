 ...
 *
 *  @method freqToMidi
 *  @param  {Number} frequency A frequency, for example, the "A"
 ...
 *  a format that is compatible with the client's web browser.
 ...
 *  web browsers. If you want to ensure compatibility, instead of a single
...
function () {
  ...
  listener3d_createClass(Listener3D, [{
    ...
    //   * @param  {Number} time
    //   * @return {Array}       All orientation params
    //   */
...
function (_Effect) {
  ...
  panner3d_createClass(Panner3D, [{
    ...
    /**
     * Maximum distance between the source and the listener
     * @method  maxDist
     * @for p5.Panner3D
     * @param  {Number} maxDistance
...
function () {
  ...
  soundRecorder_createClass(SoundRecorder, [{
     ...
     *  transferred to the p5.SoundFile.
     *
     *  @method  record
     *  @for p5.SoundRecorder
...
/**
 *  A gain node is useful to set the relative volume of sound.
 ...
 *    mixGain = new p5.Gain();
 *    mixGain.connect();
 *    sound1.disconnect(); // disconnect from p5 output
 ...
 *      return;
 *    }
 *    // map the horizontal position of the mouse to values usable for volume    *  control of sound1
 *    var sound1Volume = constrain(map(mouseX,width,0,0,1), 0, 1);
 *    var sound2Volume = 1-sound1Volume;
 *    sound1Gain.amp(sound1Volume);
 *    sound2Gain.amp(sound2Volume);
 *    // map the vertical position of the mouse to values usable for 'output    *  volume control'
  ...
  onsetDetect_createClass(OnsetDetect, [{
    ...
    value: function update(fftObject, callback) {
      ...
      if (this.isDetected === false) {
        if (this.energy - this.penergy > this.threshold) {
          this.isDetected = true;
