...
// =============================================================================
  ...
  p5.prototype.Env = function(){
  };

  p5.prototype.Env.prototype.play = function(input, attackTime, attackLevel, sustainTime, sustainLevel, releaseTime){
    // assume we're talking about output gain, unless given a different audio param
    if (input.output !== undefined){
      input = input.output.gain;
    }
    var now =  p5sound.audiocontext.currentTime;
    input.linearRampToValueAtTime(0, now);
    input.linearRampToValueAtTime(attackLevel, now + attackTime);
    input.linearRampToValueAtTime(sustainLevel, now + attackTime + sustainTime);
    input.linearRampToValueAtTime(0, now + attackTime + sustainTime + releaseTime);
  };
