...
// =============================================================================
  ...
  p5.prototype.createSelect = function() {
    ...
    self.option = function(name, value, newName, newValue) {
      if(arguments.length > 2) {
        value = value.toString();
        name = name.toString();
        //search for existing option
        for (var i = 0; i < this.elt.length; i++) {
          //if it exists change its name and possibly value
          if (value=== this.elt[i].value && name === this.elt[i].innerHTML) {
            this.elt[i].innerHTML = newName;
            if(arguments.length === 4) {
              this.elt[i].value = newValue;
            }
          }
        }
      }
      //otherwise make a new option
      else {
