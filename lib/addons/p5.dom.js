...
// =============================================================================
   ...
   *  p5.sound object. Connects to master output if no element is provided,
   *  and when this method is first called.
   *  
   *  This method is meant to be used with the p5.sound.js addon library.
   *
   *  @method  connect
   *  @param  {AudioNode|p5.sound object} audioNode [description]
   */
  p5.MediaElement.prototype.connect = function(obj) {
    ...
    if (typeof p5.prototype.getAudioContext === 'function') {
      ...
      throw 'The connect method is meant to be used with the p5.sound.js library'
    }

    // create a Web Audio MediaElementAudioSourceNode if it doesnt already exist
    if (!this.audioSourceNode) {
      this.audioSourceNode = audioContext.createMediaElementSource(this.elt);

      // connect to master output when this method is first called (otherwise, silence)
      this.audioSourceNode.connect(p5.soundOut.input);
    }
    ...
    // otherwise connect to p5.soundOut (master output of p5.sound AudioContext)
    else {
      this.audioSourceNode.connect(p5.soundOut.input);
    }
  ...
  /**
   *  Disconnect all connections, including to master output.
   *  
   *  @method  disconnect
   */
  ...
  p5.MediaElement.prototype.setTimeline = function(callback, time) {
    ...
    if (!this.elt.ontimeupdate) {
      ...
      // this.elt.onended = this._resetPrevTime.bind(this);
    }
  ...
  p5.MediaElement.prototype.clearTimelines = function(id) {
    this._timelines = [];
    this.elt.ontimeupdate = null;
  };
  ...
  p5.MediaElement.prototype._onTimeUpdate = function() {
    ...
    for (var i = 0 ; i < this._timelines.length; i++) {
      var cbTime = this._timelines[i].time;

      if (this._prevTime < cbTime && cbTime <= time) {
        this._timelines[i].callback(cbTime);
      }
  ...
  p5.MediaElement.prototype._resetPrevTime = function() {
    this._prevTime = 0;
  };
