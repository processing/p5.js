...
// =============================================================================
  ...
  /**
   * Translates an element relative to (0, 0, 0) of the window through
   * css transforms. 
   * @method  translate
   * @param  {Number} x x-position relative to upper left of window
   * @param  {Number} y y-position relative to upper left of window
   ...
   * function setup() {
   *   var cnv = createCanvas(100,100);
   *   //translates canvas 50px to the right and 100px
   *   //below upper left corner of the window
   *   cnv.translate(50,100);
   * }
   * </code></div>
   */
  p5.Element.prototype.translate = function(x, y, z){
    this.elt.style.position = 'absolute';
    var regEx = new RegExp('rotate');
    var prevAttr = '';
    var substr = '';
    while (regEx.test(this.elt.style.transform)){
      var min = this.elt.style.transform.indexOf('rotate');
      var max = this.elt.style.transform.indexOf(')');
      if (min != -1) {
        substr = this.elt.style.transform.substring(min, max+1);
        prevAttr += substr;
        console.log(this.elt.style.transform);
        this.elt.style.transform = this.elt.style.transform.replace(substr, '');
      }
    }
    this.elt.style.transform = 'translate3d('+x+'px,'+y+'px,'+z+')';
    this.elt.style.transform += prevAttr;
    return this;
  };

  /**
   * Rotates an element through css transforms in 3 dimensional space around the
   * center origin of the element.
   * @method  rotate
   * @param  {Number} x amount of degrees to rotate the element along the x-axis
   * @param  {Number} y amount of degrees to rotate the element along the y-axis
   * @param  {Number} z amount of degrees to rotate the element along the z-axis
   * @return {p5.Element}
   * @example
   * <div><code class='norender'>
   * var cnv,
   ...
   * </code></div>
   */
  p5.Element.prototype.rotate = function(x, y, z){
    var regEx = new RegExp('translate');
    var prevAttr = '';
    var substr = '';
    while (regEx.test(this.elt.style.transform)){
      var min = this.elt.style.transform.indexOf('translate');
      var max = this.elt.style.transform.indexOf(')');
      if (min != -1) {
          substr = this.elt.style.transform.substring(min, max+1);
          prevAttr += substr;
    ...
    this.elt.style.transform = 'rotateX('+x+'deg)';
    this.elt.style.transform += 'rotateY('+y+'deg)';
    this.elt.style.transform += 'rotateZ('+z+'deg)';
    this.elt.style.transform += prevAttr;
    return this;
  };
