...
// =============================================================================
  ...
  p5.MediaElement = function(elt, pInst) {
    ...
    this._prevTime = 0;
    this._timelineEventIDCounter = 0;
  ...
  p5.MediaElement.prototype.show = p5.MediaElement.prototype.showControls;
  p5.MediaElement.prototype.hide = p5.MediaElement.prototype.hideControls;
...
// =============================================================================
  ...
  /**
   *  Schedule events to trigger during MediaElement (audio/video) playback.
   *
   *  Accepts a callback function, and a time in seconds at which to trigger
   *  the callback, and an optional param for the callback.
   *
   *  The time will be passed as the first parameter to the callback function,
   *  and param will be the second parameter.
   *
   *
   *  @method  setTimeline
   *  @param {Function} callback Name of a function that will be
   *                             called at the given time, with time
   *                             and (optionally) param as its
   *                             two parameters.
   *  @param {Number}   time     Time in seconds. This will be
   *                             passed as the first parameter to
   *                             the callback function.
   *  @param {[Object]} value    An object to be passed to the as the
   *                             second parameter to the
   *                             callback function.
   *  @return {Number} id ID of this timeline event,
   *                      useful for clearTimeline(id)
   *  @example
   *  <div><code>
   *  function setup() {
   ...
   *    audioEl.setTimeline(changeBackground, 0.50, color(255,0,0) );
   *    audioEl.setTimeline(changeBackground, 3.02, color(0,0,255) );
   *    audioEl.setTimeline(changeBackground, 2.02, color(0,255,0) );
   *  }
   *
   *  function changeBackground(time, val) {
   *    background(val);
   *  }
   *  </code></div>
   */
  p5.MediaElement.prototype.setTimeline = function(callback, time, val) {
    var id = this._timelineEventIDCounter++;

    var timeline = new TimelineEvent(callback, time, id, val);
    this._timelineEvents.push(timeline);
   ...
   *  @param  {Number} id ID of the timeline event, as returned by setTimeline
   */
  p5.MediaElement.prototype.clearTimeline = function(id) {
    for (var i = 0; i < this._timelineEvents.length; i++) {
      var timeline = this._timelineEvents[i];
      if (timeline.id === id) {
   ...
   *  via the setTimeline method.
   *
   *  @method  clearTimelines
   */
  p5.MediaElement.prototype.clearTimelines = function() {
    this._timelineEvents = [];
    this.elt.ontimeupdate = null;
  };

  // private method that checks for timelineEvents to be fired if events
  // have been scheduled using setTimeline(callback, time).
  p5.MediaElement.prototype._onTimeUpdate = function() {
    var playbackTime = this.time();

    for (var i = 0 ; i < this._timelineEvents.length; i++) {
      var callbackTime = this._timelineEvents[i].time;
      var val = this._timelineEvents[i].val;


      if (this._prevTime < callbackTime && callbackTime <= playbackTime) {

        // pass the scheduled callbackTime as parameter to the callback
        this._timelineEvents[i].callback(callbackTime, val);
      }
  ...
  // TimelineEvent inspired by Tone.js Transport, MIT License Yotam Mann 2015 tonejs.org
  var TimelineEvent = function(callback, time, id, val) {
