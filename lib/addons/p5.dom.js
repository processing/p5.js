...
(function(root, factory) {
  ...
  p5.prototype._wrapElement = function(elt) {
    ...
    if (elt.tagName === 'INPUT' && elt.type === 'checkbox') {
      const converted = new p5.Element(elt, this);
      ...
      children.length > 0 &&
      children.every(c => c.tagName === 'INPUT' || c.tagName === 'LABEL')
    ) {
  ...
  tags.forEach(function(tag) {
    const method = 'create' + tag.charAt(0).toUpperCase() + tag.slice(1);
    ...
    self.selected = function(value) {
      ...
      if (arguments.length > 0) {
        ...
        if (this.elt.getAttribute('multiple')) {
          for (let i = 0; i < this.elt.selectedOptions.length; i++) {
  ...
  p5.prototype.createRadio = function(existing_radios) {
    ...
    if (radios.length > 1) {
      ...
      let prev = radios[0].name;
      let current = radios[1].name;
      count = 1;
    ...
    self.selected = function(value) {
      let i;
      const inputChildren = self._getInputChildrenArray();
      if (value) {
  ...
  p5.prototype.createColorPicker = function(value) {
    ...
    self = addElement(elt, this); // eslint-disable-line prefer-const
  ...
  function createMedia(pInst, type, src, callback) {
    ...
    const c = addElement(elt, pInst, true);
  ...
  p5.prototype.createCapture = function() {
    ...
    const c = addElement(elt, this, true);
    c.loadedmetadata = false;
    // set width and height onload metadata
    elt.addEventListener('loadedmetadata', function() {
      elt.play();
  ...
  p5.Element.prototype.style = function(prop, val) {
    ...
    if (typeof val === 'undefined') {
      if (prop.indexOf(':') === -1) {
        const styles = window.getComputedStyle(self.elt);
        const style = styles.getPropertyValue(prop);
        return style;
      } else {
      ...
      if (prop === 'rotate' || prop === 'translate' || prop === 'position') {
        const trans = Array.prototype.shift.apply(arguments);
        const f = this[trans] || this['_' + trans];
  ...
  p5.MediaElement.prototype.loadPixels = function() {
    ...
    if (this.loadedmetadata) {
      ...
      const currentTime = this.elt.currentTime;
