...
이 문서를 읽으면서, 이러한 의문점이 들 수 있습니다: "_ES5에서 ES6로 전환하면서 해당 기능을 사용하는 요점은 무엇인가요?_" 이 적절한 질문에 대한 답은 앞으로 몇 년 안에 ES5가 ES6로 대체 될 것이라는 것입니다. 그리고 아마 시간이 지나면 라이브러리를 ES5로 다시 트랜스파일링 할 필요가 없을것입니다. ES6 표준을 사용하면 코드가 간단해지고, 가독성을 높이며, 새로운 개발자의 진입 장벽을 낮춤으로써 새로운 기능을 통해 p5가 이점을 얻을 수 있습니다. 가장 중요한 것은 이것이 코드베이스의 미래를 보장한다는 것입니다.
