...
- [عملية البناء](#عملية-البناء)
  ...
  - [المهام المتنوعة](#المهام-المتنوعة)
...
أحيانًا، قد يصبح عدد الـIssues والـPull Requests التي تحتاج إلى مراجعة أمرًا مرهقًا بعض الشيء. وبينما نحاول وضع عمليات تجعل الأمور أسهل، إليك بعض النصائح والحيل التي يمكنك الاستفادة منها عند مراجعة الـIssues والـPull Requests.


### قوالب الردود

ميزة مفيدة في GitHub يمكنك استخدامها هي ميزة [الردود المحفوظة (Saved Replies)](https://docs.github.com/en/get-started/writing-on-github/working-with-saved-replies/about-saved-replies)، والتي تكون متاحة عند كتابة رد على أحد الـIssues أو الـPull Requests. بعض الإجراءات التي تم ذكرها في العمليات السابقة تتطلب الرد على الـIssues أو الـPull Requests بردود متشابهة جدًا (مثل توجيه الأسئلة إلى المنتدى، أو الموافقة على Issue للإصلاح، وغيرها)، واستخدام الردود المحفوظة يجعل هذه المهام أسرع وأكثر كفاءة.

فيما يلي بعض الردود المحفوظة التي يستخدمها مسؤولو p5.js. يمكنك استخدامها بنفسك أو إنشاء ردودك الخاصة!


##### إغلاق: لا يمكن إعادة الإنتاج

> لم نتمكن من إعادة إنتاج هذه المشكلة، لكن لا تتردد في إعادة فتحها إذا تمكنت من توفير نموذج كود يوضح المشكلة. شكرًا!


##### إغلاق: بحاجة إلى كود

> سأقوم بإغلاق هذا للتنظيم. يرجى إعادة فتحه إذا تمكنت من تقديم نموذج كود يوضح المشكلة. شكرًا!


##### إغلاق: استخدم المنتدى

> الـIssues على GitHub هنا مخصصة للأخطاء والمشاكل المتعلقة بمكتبة p5.js نفسها. بالنسبة للأسئلة حول كتابة الأكواد الخاصة بك أو الاختبارات أو اتباع الشروحات التعليمية، فإن [المنتدى](https://discourse.processing.org/) هو المكان الأفضل للنشر. شكرًا!


##### إغلاق: GSOC

> شكرًا! أفضل مكان لمناقشة مقترحات GSOC هو [المنتدى](https://discourse.processing.org/c/summer-of-code).


##### إغلاق: الإتاحة

> لا نرى اهتمامًا كبيرًا بهذه الميزة، كما أنه ليس لدينا تفسير واضح لكيفية [توسيع الإتاحة](./access.md)، لذا سأغلق هذا في الوقت الحالي. إذا كان من الممكن إضافة بيان حول الوصول إلى الطلب، فلا تتردد في إعادة فتحه.

> لم نرَ تفسيرًا إضافيًا حول كيفية [توسيع الإتاحة](./access.md)، لذا سأغلق هذا الطلب حاليًا. إذا تمت إضافة بيان وصول أكثر تفصيلًا إلى طلب الميزة، فلا تتردد في إعادة فتحه. شكرًا!


##### إغلاق: مكتبة إضافية

> نعتقد أن هذه الوظيفة تتجاوز نطاق واجهة p5.js (نحاول الحفاظ عليها بأقل حد ممكن)، لكنها قد تكون نقطة انطلاق رائعة لمكتبة إضافية. يمكنك الاطلاع على الوثائق هنا لمعرفة كيفية إنشاء مكتبة إضافية: [إنشاء مكتبة إضافية](./creating_libraries.md)


##### إغلاق PR: يجب فتح Issue أولًا

> شكرًا لك. للتذكير، يجب فتح Issue قبل فتح Pull Request وربط الـPR بهذه الـIssue. هذا ضروري لتتبع التطوير والحفاظ على وضوح النقاش. شكرًا!


##### الموافقة على Issue للإصلاح

> يمكنك المضي قدمًا في إصلاحها. شكرًا.
...
مراجعة الـPull Requests المعقدة يمكن أن تكون صعبة، خصوصًا عند الحاجة لاستخدام أوامر Git معقدة للحصول على نسخة الكود الخاصة بالـPR محليًا لاختبارها. لحسن الحظ، أداة [GitHub CLI](https://cli.github.com/) توفر حلًا بسيطًا وفعالًا لهذه المشكلة، بالإضافة إلى توفير العديد من الميزات الأخرى.

#### الخطوات الأساسية

1. **تثبيت الأداة وتسجيل الدخول**  
   بعد تثبيت GitHub CLI وتسجيل الدخول إلى حسابك على GitHub.

2. **جلب الـPR محليًا**  
   يمكنك مراجعة أي Pull Request محليًا عبر الأمر التالي:
   ```
   gh pr checkout [pull_request_id]
   ```
   هذا الأمر يقوم تلقائيًا بـ:
   - جلب (fetch) الفرع من الريبو البعيد (remote fork).
   - إنشاء فرع محلي بنفس اسم الفرع في الـPR.
   - التبديل إلى الفرع الجديد.

3. **العودة إلى الفرع الرئيسي**  
   بعد الانتهاء من المراجعة، يمكنك العودة إلى الفرع الرئيسي عبر:
   ```
   git checkout main
   ```

#### إضافة تعليقات من خلال الـCLI

يمكنك حتى **إضافة تعليقات على الـPR** مباشرة من خلال الـCLI دون الحاجة لفتح صفحة الـPR في المتصفح، مما يوفر وقتك ويسهل عملية المراجعة.

#### أوامر إضافية

أداة GitHub CLI تحتوي على **مجموعة كبيرة من الأوامر** المفيدة لإدارة الريبو والـIssues والـPull Requests، وقد تجد بعضها مناسبًا حسب طريقة عملك.

#### الخلاصة

بغض النظر عن أسلوبك في العمل، فإن GitHub CLI أداة قوية يُنصح بوجودها في بيئة العمل الخاصة بك لتسهيل التعامل مع الـRepositories بشكل أكثر كفاءة.


أكيد، إليك الترجمة مع **تنسيق أوضح** وتحسين في العرض:

---

### إدارة الإشعارات

بدلًا من مراقبة تبويب "الـIssues" أو "الـPull Requests" يدويًا في الريبو بحثًا عن أي جديد، يمكنك **"متابعة" (Watch)** الريبو بسهولة عبر الضغط على زر **"Watch"** الذي يحتوي على أيقونة العين في أعلى صفحة الريبو، بجانب اسم الريبو.

![صورة مقصوصة لزاوية صفحة ريبو على GitHub، تعرض أزرارًا في الأعلى من اليسار لليمين: Sponsor، Watch، Fork، Starred.](images/github-repo-metrics.png)

### ماذا يحدث عند المتابعة (Watch)

عند متابعة الريبو، ستصلك إشعارات بكل الأحداث التي اشتركت فيها، مثل:

- **فتح Issues جديدة.**
- **فتح Pull Requests جديدة.**
- **الإشارة إليك (mentions) باسم المستخدم الخاص بك.**
- **وغيرها من الأنشطة المرتبطة بالريبو.**

كل هذه الإشعارات ستظهر في **[صفحة الإشعارات](https://github.com/notifications)** الخاصة بك على GitHub، والتي يمكنك من خلالها:

- **وضع علامة "مقروء" على الإشعارات.**
- **أو تجاهل الإشعارات مثل البريد الوارد.**

### إشعارات البريد الإلكتروني

في بعض الحالات، قد يصلك أيضًا **إشعارات عبر البريد الإلكتروني** حول أنشطة الريبو الذي تتابعه.  
يمكنك تخصيص هذه الإشعارات (أو حتى إيقافها بالكامل) من خلال [صفحة إعدادات الإشعارات](https://github.com/settings/notifications).

### نصيحة للإعداد الأولي

لضمان توازن جيد بين متابعة النشاط وعدم التشتت بسبب كثرة الإشعارات، يُفضل أن يقوم المسؤولون (stewards) بمتابعة هذا الريبو بهذه الإعدادات:

- متابعة (Watch) **الـIssues والـPull Requests فقط**.
- ضبط إشعارات البريد الإلكتروني إلى:  
  **"المشاركة (Participating)" و"الإشارات (@mentions)" و"مخصصة (custom)"**.
