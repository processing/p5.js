...
在模板中，有这样一行文字：`解决 #[在此添加问题编号]`，你需要将 `[在此添加问题编号]` 替换为你正在处理/修复的问题的问题编号 [看上面](#关于issue)（例如，`解决 #1234`）。这样可以确保拉取请求合并后，该问题会自动关闭。如果你不希望拉取请求合并后自动关闭该问题（可能因为有更多的更改将在其他拉取请求中提交），请将 `解决` 改为 `处理`。
