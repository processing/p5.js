...
当您查看 p5.js 的源代码时，您会看到许多库中的行都是参考注释；它们看起来像这样：
...
它们通常后面跟着定义函数的实际 JavaScript 代码。参考注释始终以 `/**` 开始并以 `*/` 结束，两者之间的每一行都以 `*` 开头。

以这种方式在块中的任何内容都将被解释为参考文献。您可能通过 [JSDoc](https://jsdoc.app/) 熟悉了这种样式的代码注释。虽然 p5.js 不使用 JSDoc，但它使用了一个非常相似的工具，叫做 [YUIDoc](https://yui.github.io/yuidoc/)，它具有非常相似的参考语法。在这种参考注释样式中，每个注释块进一步分成各个元素，我们将在下面看一下。
...
如果两个签名之间唯一的区别是添加了一个可选参数，则不必创建单独的签名。如果可能的话，请限制使用此功能，因为它可能会在参考文献中创建不必要的噪音。
...
如果您不希望参考页面执行示例代码（即，您只希望代码显示出来），请在 `<div>` 中包含 “`norender`” 类：
...
如果您的示例使用外部资产文件，请将它们放入 [/docs/yuidoc-p5-theme/assets](https://github.com/processing/p5.js/tree/main/docs/yuidoc-p5-theme/assets) 文件夹中（或者重用其中已有的文件），然后在代码中使用 "assets/filename.ext" 链接到它们。请参阅 [tint()](http://p5js.org/reference/#/p5/tint) 参考示例。
...
通过以上方法，您应该拥有编写和编辑 p5.js 参考注释所需的大部分工具。然而，还有一些 JSDoc 样式参考注释的更多专门用法，您可能会在 p5.js 中遇到。这些在某些情况下很有用，但通常不是您经常需要的东西。
...
- 为了持续对参考文献进行工作，您可以运行以下命令。

```
npm run docs:dev
```

这将启动一个渲染参考文献的实时预览，每次您进行更改时都会更新（您需要在进行更改后刷新页面才能看到它们）。这是有用的，特别是用于预览在浏览器中运行的示例代码。

- 主要的模板文件存储在 `docs/` 文件夹中，在大多数情况下，您不应直接更改此文件夹中的文件，除非是要在 `docs/yuidoc-p5-theme/assets` 文件夹中添加新的资产文件。
