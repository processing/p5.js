...
p5.prototype._normalizeArcAngles = (start, stop, width, height, correctForScaling) => {
  ...
  separation = Math.abs(start - stop);

  if (correctForScaling) {
    const atanRatio = (angle, offset) =>
      Math.atan((width * Math.tan(angle - offset)) / height) + offset;

    start = start <= constants.HALF_PI ? Math.atan((width * Math.tan(start)) / height) :
      start <= 3 * constants.HALF_PI ? atanRatio(start, constants.PI) :
      atanRatio(start, constants.TWO_PI);

    stop = stop <= constants.HALF_PI ? Math.atan((width * Math.tan(stop)) / height) :
      stop <= 3 * constants.HALF_PI ? atanRatio(stop, constants.PI) :
      atanRatio(stop, constants.TWO_PI);
  }

  if (start > stop) stop += constants.TWO_PI;

  return { start, stop, correspondToSamePoint: separation < epsilon };
};
