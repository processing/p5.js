...
p5.prototype._normalizeArcAngles = (
  ...
  if (correctForScaling) {
    if (start <= constants.HALF_PI) {
      start = Math.atan((width * Math.tan(start)) / height);
    } else if (start > constants.HALF_PI && start <= 3 * constants.HALF_PI) {
      start =
        Math.atan((width * Math.tan(start - constants.PI)) / height) +
        constants.PI;
    } else {
      start =
        Math.atan((width * Math.tan(start - constants.TWO_PI)) / height) +
        constants.TWO_PI;
    }
    if (stop <= constants.HALF_PI) {
      stop = Math.atan((width * Math.tan(stop)) / height);
    } else if (stop > constants.HALF_PI && stop <= 3 * constants.HALF_PI) {
      stop = Math.atan((width * Math.tan(stop - constants.PI)) / height) + constants.PI;
    } else {
      stop =
        Math.atan((width * Math.tan(stop - constants.TWO_PI)) / height) +
        constants.TWO_PI;
    }
  }
