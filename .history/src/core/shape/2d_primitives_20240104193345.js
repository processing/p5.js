...
p5.prototype._normalizeArcAngles = (start, stop, width, height, correctForScaling) => {
  ...
  if (correctForScaling) {
    ...
    start =
      start <= constants.HALF_PI
        ? Math.atan((width * Math.tan(start)) / height)
        : start <= 3 * constants.HALF_PI
        ? atanRatio(start, constants.PI)
        : atanRatio(start, constants.TWO_PI);

    stop =
      stop <= constants.HALF_PI
        ? Math.atan((width * Math.tan(stop)) / height)
        : stop <= 3 * constants.HALF_PI
        ? atanRatio(stop, constants.PI)
