suite('stroke WebGL', function() {
  ...
  suite('default stroke shader', function() {
    test('check default shader creation', function(done) {
      ...
      assert(myp5._renderer.curStrokeShader === myp5._renderer._getLineShader(),
        'default stroke shader was not initialized with GL canvas');
      assert(myp5._renderer.curFillShader === myp5._renderer._getColorShader(),
    ...
    test('check activate and deactivating fill and stroke', function(done) {
      myp5.noStroke();
      assert(myp5._renderer.curStrokeShader.active === false,
        'stroke shader still active after noStroke()');
      assert.isTrue(myp5._renderer.curFillShader.active === true,
        'fill shader deactivated by noStroke()');
      myp5.stroke(0);
      myp5.noFill();
      assert(myp5._renderer.curStrokeShader.active === true,
        'stroke shader not active after stroke()');
      assert.isTrue(myp5._renderer.curFillShader.active === false,
